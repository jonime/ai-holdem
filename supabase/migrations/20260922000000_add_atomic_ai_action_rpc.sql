create or replace function public.apply_ai_action_if_version(
  ...
  p_ai_state jsonb,
  p_legal_actions jsonb,
  p_choice text,
  p_probabilities jsonb,
  p_confidence numeric,
  p_raise_size_choice text,
  p_raise_size_probabilities jsonb,
...
begin
  ...
  with eligible_action as (
    ...
    select
      ...
      p_state_after
    from updated_game
    returning hand_id, player_id, sequence
    ...
    where p_hand_complete
      and id in (select hand_id from inserted_decision)
  )
  select
    id,
    status,
